...
const addReqBtn = document.querySelector('.addReqBtn');
...
addReqBtn.addEventListener('click', () => {
  ...
  const EmpReq = document.querySelector('#EmpReq');
  ...
  const Timestamp = document.querySelector('#reqTimestamp');
...
// for Qr code btn 
jQuery('#frmAddReq').on('submit', function (e) {
  e.preventDefault();
  jQuery.ajax({
    url: 'https://script.google.com/macros/s/AKfycbzRxKNIHTYx_-aiNjYRWjuwFeeXtZw-kkJB_bpjByG3U4DmmYBrUzMbghmY7oG06Hh-2A/exec',
    type: 'post',
    data: jQuery('#frmAddReq').serialize(),
    beforeSend: function () {
